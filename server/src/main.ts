...
async function bootstrap(): Promise<void> {
   ...
   * Cookie parser
   *
   * Required when refresh tokens or other values
  ...
  const swaggerConfig = new DocumentBuilder()
    ...
    .setDescription(
      "Backend API for the Mini Facebook assignment",
    )
    .setVersion("1.0.0")
    .addBearerAuth()
    .build();
  ...
  /*
   * Railway provides its own PORT environment variable.
   *
   * 0.0.0.0 ensures that the application accepts
   * external traffic inside Railway.
   */
  ...
  if (nodeEnv !== "production") {
    ...
    console.log(
      `Swagger documentation: ${appUrl}/docs`,
    );
  }
}

function createSwaggerThemeCss(): string {
  return `
...
function createSwaggerThemeScript(): string {
  return `
    (function () {
